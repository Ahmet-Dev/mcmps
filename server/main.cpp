...
public:
    // taskMode: "single" -> tek programın kodu, "all" -> tüm programların kodları
    ServerSession(boost::asio::ssl::stream<tcp::socket> socket, const std::string &taskMode)
        : socket_(std::move(socket)), taskMode_(taskMode) { }
...
class Server {
public:
    Server(boost::asio::io_context &io_context, int port, boost::asio::ssl::context &ssl_context, const std::string &taskMode)
        : acceptor_(io_context, tcp::endpoint(tcp::v4(), port)),
          ssl_context_(ssl_context), taskMode_(taskMode)
    {
        doAccept();
    }
            ...
            [this](boost::system::error_code ec, tcp::socket socket) {
                if (!ec) {
                    ...
                    std::make_shared<ServerSession>(
                        boost::asio::ssl::stream<tcp::socket>(std::move(socket), ssl_context_), taskMode_
                    )->start();
                } else {
...
int main() {
    try {
        ...
        // "single" modunda tek bir programın kodu, "all" modunda tüm programların kod paketleri gönderilir.
        std::string taskMode = "single";
        Server server(io_context, 1881, ssl_context, taskMode);
        io_context.run();
